...
available_files <- list.files("~/fs_ed/SERDP_Project/Cogongrass_trait_sampling/root_respiration_2019/" )
...
### Fine roots
Cogongrass_Respiration_Biomass_2019 <- read_excel("~/fs_ed/SERDP_Project/Cogongrass_trait_sampling/root_respiration_2019/Cogongrass_Respiration_Biomass_2019.xlsx")
...
for (i in seq_along(fine_root_files)){
  ...
  mass <- Cogongrass_Respiration_Biomass_2019$`Fine Root Dry g`[Cogongrass_Respiration_Biomass_2019$Number == number] %>% 
    as.numeric()
  file_path <- paste("~/fs_ed/SERDP_Project/Cogongrass_trait_sampling/root_respiration_2019/", file_name, sep = "")
  ...
  Efflux <- Efflux * 0.008 * 1000 # Units error fix. 0.0001 m^2/80 cm^2 to cancel out area, and translate into kilograms in the what the soil core returns
  #plot(Efflux)
...
for(i in seq_along(rhizome_files)){
  ...
  #Efflux[Efflux >= 3 | Efflux <= -1] <- NA # Not possible, and/or a leak
  Efflux <- na.omit(Efflux)
  Efflux <- Efflux * 0.008 * 1000# Units error fix.80 cm^2 to cancel out area in the what the soil core returns
...
for(i in seq_along(combo_files)){
  ...
  Efflux[Efflux >= 3 | Efflux <= -1] <- NA # Not possible, and/or a leak
  Efflux <- na.omit(Efflux)
  Efflux <- Efflux * 0.008 * 1000# Units error fix. 0.0001 m^2/80 cm^2 to cancel out area in the what the soil core returns
  plot(Efflux, main = file_name)
