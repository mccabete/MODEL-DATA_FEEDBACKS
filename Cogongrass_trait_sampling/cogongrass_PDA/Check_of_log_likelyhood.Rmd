...
/fs/data2/output//PEcAn_1000019795/pft/cogongrass2/ : Now /projectnb/dietzelab/pecan.data/output/tmccabe/model_data_feedbacks/test_pecan/PEcAn_1000019795

/projectnb/dietzelab/pecan.data/output//tmccabe/1000019795/ss.pdaPDA_50kn_MODEL_REDO_after_fix_of_obs.Rdata

# 200 knots

/fs/data2/output//PEcAn_PDA_for_PF/pft/cogongrass2/: Now /projectnb/dietzelab/pecan.data/output/tmccabe/model_data_feedbacks/test_pecan/PEcAn_PDA_for_PF

/projectnb/dietzelab/pecan.data/output//tmccabe//ss.pdaPDA_for_PF.Rdata

## Load up the observational data, and format it so that it can be directly compared to model outputs
```{r}
## The ED2 data will be in KgC/m^2. 
library(here)
biomass <- read.csv(here("Cogongrass_trait_sampling", "allometries", "Cogongrass_Biomass_datasheet_2019.csv")) ## Data in g biomass

biomass$Quadrat.area <- 30^2 # 30 cm^2
...
drought <- dplyr::filter(biomass, biomass$Treatment == "Drought") 
ambient <- dplyr::filter(biomass, biomass$Treatment == "Ambient")
#ambient <- na.omit(ambient)


## Santity check of units

# hist(log10(rlnorm(1000,0.811,0.843)))
# points(x = ambient$fine_roots_corrected_area/(ambient$tillars_corrected + ambient$leaves_corrected), y = rep(0, length(ambient$fine_roots_corrected_area/(ambient$tillars_corrected + ambient$leaves_corrected))))


## Units in g biomass /cm^2. First put in Kg Biomass / m^2
...
agb <- ambient$tillars_corrected + ambient$leaves_corrected
bgb <- ambient$fine_roots_corrected_area + ambient$rhizome_corrected_area


obs <- list()
obs[[1]] <- data.frame(AbvGrndWood = agb[1], posix = c("2019-03-02 00:00:00"))
obs[[2]] <- data.frame(TotSoilCarb = bgb[1], posix = c("2019-03-02 00:00:00"))

obs_multi <- list()
obs_multi[[1]] <- data.frame(AbvGrndWood = agb, posix = c("2019-03-02 00:00:00")) ## Model output for 5/30 not generated. Maybe march is the earliest day? 
obs_multi[[2]] <- data.frame(TotSoilCarb = bgb, posix = c("2019-03-02 00:00:00"))
...
```{r}
library(tidyverse)
load("/projectnb/dietzelab/pecan.data/output//tmccabe//ss.pdaPDA_for_PF.Rdata") ## Get SS

#attach("/projectnb/dietzelab/pecan.data/output/tmccabe/model_data_feedbacks/test_pecan//PEcAn_1000019795/pft/cogongrass2/prior.distns.Rdata") ## Get Priors
#attach("~/Cogongrass_trait_sampling/cogongrass_PDA/PEcAn_1000019795/pft/cogongrass2/")
load("/usr3/graduate/tmccabe/test/MODEL-DATA_FEEDBACKS/Cogongrass_trait_sampling/cogongrass_PDA/PEcAn_1000019795/pft/cogongrass2/prior.distns.Rdata")
prior_pre_pda <- prior.distns
rm(prior.distns)

## Load up model estimates
outfile <- "/projectnb/dietzelab/pecan.data/output/tmccabe/model_data_feedbacks/test_pecan//PEcAn_PDA_for_PF/out/"
...
if(!is_null(sus_knots)){
  ...
  for(i in sus_knots){
    hist_path <- paste0("/projectnb/dietzelab/pecan.data/output/tmccabe/model_data_feedbacks/test_pecan//PEcAn_", id, "/out/", name, ".knot.", i, "/history.xml")
...
for(i in seq_along(SS_reconstruct$water_conductance)){
  history_path <- paste0("/projectnb/dietzelab/pecan.data/output/tmccabe/model_data_feedbacks/test_pecan//PEcAn_", id, "/out/", name, ".knot.", i, "/history.xml")
...
for(i in 1:length(SS_reconstruct$growth_respiration)){
  ...
  BGBlike[i] = sum(dnorm(na.omit(obs_multi[[2]]$TotSoilCarb), biomass$bgb[i], bgb_sd, log = TRUE))
...
lines(density(log(rlnorm(100000, -5.400 ,3.00e+00)) ), main = "Water conductance prior", col = "#2CBEC5")
legend("topleft", 
...
lines(density(rlnorm(100000, -5.400 ,3.00e+00) ), main = "Water conductance prior", col = "#2CBEC5")
legend("topleft", 
  legend = c("Posterior", "Prior"), 
  fill = c("#7570b3", "#2CBEC5"))
...
lines(density(rbeta(10000, 2.630, 6.52e+00)),col = "#2CBEC5" )
legend("topleft", 
  legend = c("Posterior", "Prior"), 
  fill = c("#7570b3", "#2CBEC5"))
...
save(post.distns, file = "/projectnb/dietzelab/pecan.data/output/tmccabe/model_data_feedbacks/test_pecan//PEcAn_PDA_for_PF/pft/cogongrass2/PDA_MA_post.Rdata")
```
...
## Growth Respiration
growth_resp_prior <- quantile(rbeta(7000000,MA_prior$parama[2], MA_prior$paramb[2]), probs = c(0.025, 0.975))
growth_resp_post <-  quantile(rbeta(7000000,pda_post$parama[2], pda_post$paramb[2]), probs = c(0.025, 0.975))
growth_reduction <- (growth_resp_prior[2] - growth_resp_prior[1]) / (growth_resp_post[2] - growth_resp_post[1])

plot(density(rbeta(1000000,pda_post$parama[2], pda_post$paramb[2])), main = "Growth Respiration Factor", col =  "#7570b3", sub = paste("A", round(growth_reduction, 2), "fold decrease in 95% CI"))
lines(density(rbeta(1000000,MA_post$parama[2], MA_post$paramb[2])), col = "#2CBEC5")
legend("topleft", 
  legend = c("Posterior", "Prior"), 
  fill = c("#7570b3", "#2CBEC5"))
...
wat_con_prior <- quantile(log(rlnorm(1000000,MA_prior$parama[10], MA_prior$paramb[10])), probs = c(0.025, 0.975))
wat_con_post <- quantile(log(rlnorm(1000000,pda_post$parama[10], pda_post$paramb[10])), probs = c(0.025, 0.975))

wat_con_prior_linear <- quantile(rlnorm(1000000,MA_prior$parama[10], MA_prior$paramb[10]), probs = c(0.025, 0.975))
wat_con_post_linear <- quantile(rlnorm(1000000, pda_post$parama[10], pda_post$paramb[10]), probs = c(0.025, 0.975))

wat_con_red <- (wat_con_prior[2] - wat_con_prior[1])/ (wat_con_post[2] - wat_con_post[1])
wat_con_red_linear <- (wat_con_prior_linear[2] - wat_con_prior_linear[1])/ (wat_con_post_linear[2] - wat_con_post_linear[1])

## Water Conductance log scale
plot(density(log(rlnorm(1000000,pda_post$parama[10], pda_post$paramb[10]))), main = " Log-scale Water Conductance Factor", col = "#7570b3", sub = paste("A", round(wat_con_red, 2), "fold decrease in 95% CI"))
lines(density(log(rlnorm(1000000,MA_prior$parama[10], MA_prior$paramb[10]))), col = "#2CBEC5" )
legend("topleft", 
  legend = c("Posterior", "Prior"), 
  fill = c("#7570b3", "#2CBEC5"))

## Water Conductance liner scale
plot(density(rlnorm(1000000,pda_post$parama[10], pda_post$paramb[10])), main = " Linear-scale Water Conductance Factor", col = "#7570b3", sub = paste("A", round(wat_con_red_linear, 2), "fold decrease in 95% CI"))
lines(density(rlnorm(1000000,MA_prior$parama[10], MA_prior$paramb[10])), col = "#2CBEC5" )
legend("topleft", 
  legend = c("Posterior", "Prior"), 
  fill = c("#7570b3", "#2CBEC5"))


print("Water conductace had a mean of",round(mean(rlnorm(1000000,MA_prior$parama[10], MA_prior$paramb[10])), 2), ", and is ", round(mean(rlnorm(1000000,MA_prior$parama[10], MA_prior$paramb[10])), 2), "with constraint.")
```
