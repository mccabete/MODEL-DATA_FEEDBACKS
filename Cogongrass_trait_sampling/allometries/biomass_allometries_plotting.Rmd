...
library(ggplot2)
wd <- getwd()
biomass <- read.csv(paste0( wd, "/Cogongrass_Biomass_datasheet_2019.csv"))
```

## Plots

Bety database fineroot2leaf trait is calcualated in this code chunk. I decided not to include rhizomes in my fineroot2leaf train, because I don't think they qualify as fine roots. 
...
#### BETY TRAIT CALCULATIONS
ambient_fineroot2leaf <- ambient$fine_roots_corrected_area/ ambient$leaves_corrected
ambient_fineroot2leaf_plusrhizome <- (ambient$fine_roots_corrected_area + (ambient$dry.biomass.rhizome/ambient$Soil.Core.cross.sectional.area.cm.2)) / ambient$leaves_corrected

drought_fineroot2leaf <- drought$fine_roots_corrected_area/ drought$leaves_corrected
drought_fineroot2leaf_plus_rhizome <- (drought$fine_roots_corrected_area + (drought$rhizome_corrected_area))/ drought$leaves_corrected

print(mean(na.omit(ambient_fineroot2leaf)))
print(sd(na.omit(ambient_fineroot2leaf)))

mean(na.omit(drought_fineroot2leaf))

mean(na.omit(ambient_fineroot2leaf_plusrhizome))
mean(na.omit(drought_fineroot2leaf_plus_rhizome))

hist(log10(stats::rlnorm(10000, 0.81, 0.84))) # previous pecan prior
abline(v = log10(0.07), col = "blue") # When I used area = 30 instead of area = 30^2 ambient
abline(v = log10(0.14), col = "blue", lty = "dashed") 
abline(v = log10(2.072341), col = "red") # only fineroots2leaves, no rhizomes
abline(v = log10(4.416361), col = "red", lty = "dashed" ) # Drought only fineroots2leaves, no rhizomes
abline(v = log10(mean(na.omit(ambient_fineroot2leaf_plusrhizome))), col = "green") # ambient, rhizomes
abline(v = log10(mean(na.omit(drought_fineroot2leaf_plus_rhizome))), col = "green", lty = "dashed") # ambient, rhizomes
